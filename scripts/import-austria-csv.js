/**
 * Import Austria Images Table.csv into artImages.json.
 * Run: node scripts/import-austria-csv.js
 */
const fs = require("fs");
const path = require("path");

const ROOT = path.join(__dirname, "..");
const CSV_PATH = path.join(ROOT, "public", "assets", "Austria Images Table.csv");
const ART_IMAGES = path.join(ROOT, "src", "assets", "artImages.json");

const REGION_TO_STORY = {
  Vienna: { storyLink: "/austria/vienna", folder: "Vienna", slug: "vienna" },
  Salzburg: { storyLink: "/austria/salzburg", folder: "Salzburg", slug: "salzburg" },
  "The Wider Country": {
    storyLink: "/austria/wider-country",
    folder: "Wider Country",
    slug: "wider-country",
  },
};
...
function cloudinaryPaths(filename) {
  ...
  return {
    blog: `Austria/Small/${id}`,
    lightbox: `Austria/Full/${id}`,
    gallery: `Austria/Small/${id}z`,
    thumbnail: `Austria/Thumbnail/${id}`,
  };
}
...
function main() {
  ...
  const filtered = catalog.filter(
    (item) => !String(item.storyLink || "").startsWith("/austria/")
  );
  ...
  for (const cells of dataRows) {
    ...
    const region = cells[col.category]?.trim();
    const subCategory = cells[col["Sub-category"]]?.trim();
    ...
    entries.push({
      ...
      shopLink: `/nomads-shop/austria/${mapping.slug}?category=${encodeURIComponent(subCategory || "")}`,
  ...
  console.log(`Added ${entries.length} Austria images (${next.length} total in catalog).`);
}
