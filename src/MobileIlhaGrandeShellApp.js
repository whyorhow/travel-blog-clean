...
import { hasIlhaGrandeStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /brazil/rio/ilha-grande — static HTML hero is LCP. */
export default function MobileIlhaGrandeShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [IlhaGrandePage, setIlhaGrandePage] = useState(null);
  const staticHero = hasIlhaGrandeStaticHero();
  const importIlhaGrandePage = useCallback(() => import("./pages/IlhaGrande"), []);
  useStaticHeroPageChunkLoader(staticHero, importIlhaGrandePage, setIlhaGrandePage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/rio/ilha-grande") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {IlhaGrandePage ? <IlhaGrandePage /> : null}
            </NarrativeProvider>
          </main>
