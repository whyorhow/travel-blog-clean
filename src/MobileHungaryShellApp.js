...
import { hasHungaryStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /hungary — static HTML hero is LCP. */
export default function MobileHungaryShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasHungaryStaticHero();
  const importPage = useCallback(() => import("./pages/Hungary"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/hungary") {
