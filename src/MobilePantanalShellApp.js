...
/**
 * Mobile /brazil/pantanal only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobilePantanalShellApp({ root }) {
  ...
  useStaticHeroPageChunkLoader(staticHero, importPantanalPage, setPantanalPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/pantanal") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {PantanalPage ? <PantanalPage /> : null}
            </NarrativeProvider>
          </main>
