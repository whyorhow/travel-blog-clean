...
import { hasTennesseeStaticHero } from "./utils/staticPageHero";
import {
...
/**
 * Mobile /united-states/tennessee only — static HTML hero (outside #root) is LCP;
 * CountryLandingTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileTennesseeShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [TennesseePage, setTennesseePage] = useState(null);
  const staticHero = hasTennesseeStaticHero();
  const importTennesseePage = useCallback(
    () => import("./pages/Tennessee"),
    []
  );
  useStaticHeroPageChunkLoader(staticHero, importTennesseePage, setTennesseePage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/united-states/tennessee") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {TennesseePage ? <TennesseePage /> : null}
            </NarrativeProvider>
          </main>
