...
import { hasMemphisStaticHero } from "./utils/staticPageHero";
import {
...
/**
 * Mobile /united-states/tennessee/memphis only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileMemphisShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [MemphisPage, setMemphisPage] = useState(null);
  const staticHero = hasMemphisStaticHero();
  const importMemphisPage = useCallback(() => import("./pages/Memphis"), []);
  useStaticHeroPageChunkLoader(staticHero, importMemphisPage, setMemphisPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/united-states/tennessee/memphis") {
