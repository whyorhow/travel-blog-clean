...
const LIGHT_SECTION = "py-12 md:py-16";
const LIGHT_STACK = "flex flex-col gap-12 md:gap-16";

function LightHeroBlock({
  skipHero,
  heroConfig,
  heroPageData,
  heroImage,
  ...
  if (heroConfig) {
    return <Hero heroConfig={heroConfig} pageData={heroPageData} />;
  }
  ...
  return <Hero heroConfig={{}} pageData={heroPageData} />;
}

function LightContextBlock({
  sections,
  locationData,
  subsectionHeading,
  shouldShowContextMap,
  exploreSectionId,
}) {
  if (sections?.length > 0 && locationData.coords) {
    return (
      <SubsectionNavigator
  ...
  if (shouldShowContextMap && locationData.coords) {
    return (
...
function LightGalleryBlock({ galleryImages, galleryBackground, heading }) {
...
function LightFooterBlock({
  ...
  return (
    <section className={LIGHT_SECTION}>
      <div className={LIGHT_STACK}>
        <CloseBlock text={reflectiveClose} style={closeStyle} />

        {(returnLink || nextLink) && (
          <div className="w-full max-w-lg mx-auto flex flex-row items-stretch justify-center gap-3 sm:gap-4 px-4">
            {returnLink && (
              <Link
                to={returnLink.path}
...
function LightTemplate({
  ...
  const renderEditorial = useCallback(
    (placement, afterNarrativeIndex, className = "") => {
  ...
  const openNarrativeLightbox = useCallback((narrative) => {
    setNarrativeLightboxImage({
      image: cloudinaryImageUrl(
        narrative.image?.lightboxSrc ?? narrative.image?.src,
        {
      ...
      description:
        narrative.image?.expandDescription ??
        narrative.expandDescription ??
        narrative.paragraph ??
        "",
    });
  }, []);

  const narrativePhotoClass =
    atmosphere === "austria" ? NARRATIVE_PHOTO_JOURNAL : "";
    ...
    <>
      ...
      {narrativeLightboxImage && (
        <SimpleLightbox
          ...
          currentIndex={0}
          setCurrentIndex={(v) => {
            if (v === null) setNarrativeLightboxImage(null);
          }}
        />
      )}
      ...
      <div className={surface.wrapper} style={surface.wrapperInline}>
        ...
        <LightHeroBlock
          ...
          heroPageData={heroPageData}
          heroImage={heroImage}
          isDiaryHero={isDiaryHero}
          config={config}
          ...
          <>
            ...
            {variant === "urban" && (
              <section className="max-w-3xl mx-auto px-6 py-16 text-center">
                ...
                <p className={surface.intro}>{intro?.paragraph}</p>
              </section>
            )}
            {variant === "urban" &&
                ...
                <div className="relative w-full overflow-hidden">
                  ...
                  <div className="max-w-[1100px] mx-auto relative z-10 px-6 md:px-10 lg:px-16">
                    ...
                    <div className="section-narrative max-w-[760px] mx-auto">
                      ...
                      {intro?.paragraphs?.map((text, i) => (
                        <p
                          key={i}
                          className="text-lg sm:text-xl leading-[1.8] mb-6"
                        >
                          {text}
                        </p>
                      ))}
                  ...
                  <div className="max-w-[1100px] mx-auto px-6 md:px-12 lg:px-16 pb-16 relative z-10 text-stone-900 md:text-stone-800">
                    ...
                    {narratives?.map((narrative, i) =>
                      renderNarrativeItem(narrative, i),
                    )}
                  </div>
                </div>
            ...
            {(variant === "nature" || variant === "coastal") && (
              <>
                <section className="max-w-3xl mx-auto px-6 py-16 text-center text-stone-800">
                  ...
                  {intro?.paragraphs?.map((text, i) => (
                    <p key={i} className={`${surface.intro} mb-6`}>
                      {text}
                    </p>
                  ))}
                </section>
                  ...
                  <section className="w-full py-8">
                    <div className="max-w-6xl mx-auto px-6">
                      ...
                      {featureImage.caption && (
                        <p
                          className={`text-sm mt-4 text-center italic ${surface.muted || "text-stone-500"}`}
                        >
                          {featureImage.caption}
                        </p>
                      )}
            ...
            <LightContextBlock
              ...
              shouldShowContextMap={shouldShowContextMap}
            ...
            {renderEditorial(
              EDITORIAL_PLACEMENTS.BEFORE_GALLERY,
              undefined,
              "relative z-10 mb-8",
            )}
