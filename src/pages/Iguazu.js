...
const galleryImages = GALLERY_ORDER
  .map(id => iguazuImages.find(img => img.id === id))
  .filter(Boolean)
  .map(img => ({
...
function Iguazu() {
  return (
    <LightTemplate
      ...
      narratives={[
        {
          image: { src: resolvePublicId('iguazu16'), alt: 'Iguazu from above through the trees' },
        ...
        {
          image: { src: resolvePublicId('iguazu6'), alt: 'Up close at the falls' },
          heading: 'The Impact',
        ...
        {
          image: { src: resolvePublicId('iguazu9'), alt: 'Crossing to the Argentinian side' },
        ...
        {
          image: { src: resolvePublicId('iguazu11'), alt: 'The lingering memory of scale' },
