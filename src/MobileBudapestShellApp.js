...
import { hasBudapestStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /hungary/budapest — static HTML hero is LCP. */
export default function MobileBudapestShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasBudapestStaticHero();
  const importPage = useCallback(() => import("./pages/BudapestNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/hungary/budapest") {
