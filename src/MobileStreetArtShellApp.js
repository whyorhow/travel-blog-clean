...
import { hasStreetArtStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /brazil/saopaulo/street-art — static HTML hero is LCP. */
export default function MobileStreetArtShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasStreetArtStaticHero();
  const importPage = useCallback(() => import("./pages/Graffiti"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/saopaulo/street-art") {
