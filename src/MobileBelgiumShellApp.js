...
import { hasBelgiumStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /belgium — static HTML hero is LCP. */
export default function MobileBelgiumShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasBelgiumStaticHero();
  const importPage = useCallback(() => import("./pages/Belgium"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/belgium") {
