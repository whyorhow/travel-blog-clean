...
import { hasGreenSpacesStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /brazil/saopaulo/green-spaces — static HTML hero is LCP. */
export default function MobileGreenSpacesShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasGreenSpacesStaticHero();
  const importPage = useCallback(() => import("./pages/GreenSpaces"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/saopaulo/green-spaces") {
