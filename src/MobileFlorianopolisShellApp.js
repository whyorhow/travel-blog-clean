...
/**
 * Mobile /brazil/florianopolis only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileFlorianopolisShellApp({ root }) {
  ...
  useStaticHeroPageChunkLoader(staticHero, importFlorianopolisPage, setFlorianopolisPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/florianopolis") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {FlorianopolisPage ? <FlorianopolisPage /> : null}
            </NarrativeProvider>
          </main>
