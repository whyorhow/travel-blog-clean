...
import { hasGreeceStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /greece — static HTML hero is LCP. */
export default function MobileGreeceShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasGreeceStaticHero();
  const importPage = useCallback(() => import("./pages/Greece"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/greece") {
