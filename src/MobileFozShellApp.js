...
import { hasFozStaticHero } from "./utils/staticPageHero";
import {
...
/**
 * Mobile /brazil/foz only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileFozShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [IguazuPage, setIguazuPage] = useState(null);
  const staticHero = hasFozStaticHero();
  const importIguazuPage = useCallback(() => import("./pages/Iguazu"), []);
  useStaticHeroPageChunkLoader(staticHero, importIguazuPage, setIguazuPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/foz") {
