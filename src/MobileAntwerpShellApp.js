...
import { hasAntwerpStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /belgium/antwerp — static HTML hero is LCP. */
export default function MobileAntwerpShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasAntwerpStaticHero();
  const importPage = useCallback(() => import("./pages/AntwerpNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/belgium/antwerp") {
