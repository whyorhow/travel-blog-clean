...
import { hasMountainsStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /united-states/tennessee/mountains — static HTML hero is LCP. */
export default function MobileMountainsShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [MountainsPage, setMountainsPage] = useState(null);
  const staticHero = hasMountainsStaticHero();
  const importMountainsPage = useCallback(() => import("./pages/Mountains"), []);
  useStaticHeroPageChunkLoader(staticHero, importMountainsPage, setMountainsPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/united-states/tennessee/mountains") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {MountainsPage ? <MountainsPage /> : null}
            </NarrativeProvider>
          </main>
