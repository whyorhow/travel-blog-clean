...
/**
 * Mobile /brazil/saopaulo only — skips full App shell. Static HTML hero (outside #root) is LCP;
 * DenseTemplate skips React hero and scroll-gates the gallery.
 */
export default function MobileSaoPauloShellApp({ root }) {
  ...
  useStaticHeroPageChunkLoader(staticHero, importSaoPauloPage, setSaoPauloPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/saopaulo") {
