...
import { hasAustriaStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /austria — static HTML hero is LCP. */
export default function MobileAustriaShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasAustriaStaticHero();
  const importPage = useCallback(() => import("./pages/Austria"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/austria") {
