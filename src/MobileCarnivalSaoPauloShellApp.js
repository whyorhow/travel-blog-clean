...
import { hasCarnivalSaoPauloStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /brazil/saopaulo/carnival — static HTML hero is LCP. */
export default function MobileCarnivalSaoPauloShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasCarnivalSaoPauloStaticHero();
  const importPage = useCallback(() => import("./pages/CarnivalSaoPaulo"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/saopaulo/carnival") {
