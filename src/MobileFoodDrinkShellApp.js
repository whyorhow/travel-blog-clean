...
import { hasFoodDrinkStaticHero } from "./utils/staticPageHero";
import {
...
/**
 * Mobile /brazil/food-drink only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileFoodDrinkShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [FoodDrinkPage, setFoodDrinkPage] = useState(null);
  const staticHero = hasFoodDrinkStaticHero();
  const importFoodDrinkPage = useCallback(
    () => import("./pages/BrazilFoodDrink"),
    []
  );
  useStaticHeroPageChunkLoader(staticHero, importFoodDrinkPage, setFoodDrinkPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/food-drink") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {FoodDrinkPage ? <FoodDrinkPage /> : null}
            </NarrativeProvider>
          </main>
