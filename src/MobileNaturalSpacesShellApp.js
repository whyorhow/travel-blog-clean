...
import { hasNaturalSpacesStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /brazil/natural-spaces — static HTML hero is LCP. */
export default function MobileNaturalSpacesShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [NaturalSpacesPage, setNaturalSpacesPage] = useState(null);
  const staticHero = hasNaturalSpacesStaticHero();
  const importNaturalSpacesPage = useCallback(
    () => import("./pages/BrazilNaturalSpaces"),
    []
  );
  useStaticHeroPageChunkLoader(
    staticHero,
    importNaturalSpacesPage,
    setNaturalSpacesPage,
    10000
  );
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/natural-spaces") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {NaturalSpacesPage ? <NaturalSpacesPage /> : null}
            </NarrativeProvider>
          </main>
