...
import { hasNashvilleStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /united-states/tennessee/nashville — static HTML hero is LCP. */
export default function MobileNashvilleShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [NashvillePage, setNashvillePage] = useState(null);
  const staticHero = hasNashvilleStaticHero();
  const importNashvillePage = useCallback(() => import("./pages/Nashville"), []);
  useStaticHeroPageChunkLoader(staticHero, importNashvillePage, setNashvillePage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/united-states/tennessee/nashville") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {NashvillePage ? <NashvillePage /> : null}
            </NarrativeProvider>
          </main>
