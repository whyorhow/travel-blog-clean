...
import { hasSalzburgStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /austria/salzburg — static HTML hero is LCP. */
export default function MobileSalzburgShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasSalzburgStaticHero();
  const importPage = useCallback(() => import("./pages/SalzburgNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/austria/salzburg") {
