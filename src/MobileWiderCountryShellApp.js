...
import { hasWiderCountryStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /austria/wider-country — static HTML hero is LCP. */
export default function MobileWiderCountryShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasWiderCountryStaticHero();
  const importPage = useCallback(() => import("./pages/WiderCountryNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/austria/wider-country") {
