...
/**
 * Mobile /brazil/salvador only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileSalvadorShellApp({ root }) {
  ...
  useStaticHeroPageChunkLoader(staticHero, importSalvadorPage, setSalvadorPage, 10000);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/salvador") {
      ...
      >
        <div className="min-h-screen flex flex-col">
          ...
          <main id="main-content" className="flex-grow">
            <NarrativeProvider>
              {SalvadorPage ? <SalvadorPage /> : null}
            </NarrativeProvider>
          </main>
