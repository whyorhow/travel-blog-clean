...
/**
 * Mobile /brazil/santos only — static HTML hero (outside #root) is LCP;
 * LightTemplate skips React hero and scroll-gates below-fold content.
 */
export default function MobileSantosShellApp({ root }) {
  ...
  useEffect(() => {
    if (!staticHero) {
      import("./pages/Santos").then(({ default: Page }) => setSantosPage(() => Page));
      return undefined;
    }
    let cancelled = false;
    const loadPage = () => {
      import("./pages/Santos").then(({ default: Page }) => {
        if (!cancelled) setSantosPage(() => Page);
      });
    };
    const onScroll = () => {
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/brazil/santos") {
