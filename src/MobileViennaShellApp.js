...
import { hasViennaStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /austria/vienna — static HTML hero is LCP. */
export default function MobileViennaShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasViennaStaticHero();
  const importPage = useCallback(() => import("./pages/ViennaNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/austria/vienna") {
