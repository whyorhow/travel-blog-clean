...
const Bullet = ({ rotation = 0 }) => (
  <img
    src={bulletpoint}
    alt=""
    className="w-1.5 h-1.5 mr-2 inline-block opacity-80"
...
const SidebarMenu = ({
  ...
  const [openPortugal, setOpenPortugal] = useState(false);
    ...
    >
      ...
      <div className="ml-6 flex flex-col gap-2">
        ...
        <CountryRegionNav
          ...
          to="/portugal"
          isOpen={openPortugal}
          onToggle={() => toggleSubmenu("portugal", setOpenPortugal)}
          onNavigate={() => setMenuOpen(false)}
          bullet={<Bullet rotation={randomRot()} />}
          toggleLabel="Toggle portugal submenu"
        >
          Portugal
        </CountryRegionNav>
      </div>
