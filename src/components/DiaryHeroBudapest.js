...
import titleImg from "../assets/images/Budapest Title.webp";

export default function DiaryHeroBudapest({ heroOpenLightbox }) {
  ...
  return (
    <>
      <section className="relative w-full overflow-hidden mt-4 md:mt-0" style={{ minHeight: '100vh' }}>
        ...
        >
          ...
          >
            <img
              src={diaryImg}
              alt="Budapest diary"
              className="w-auto h-auto object-cover scale-150"
              style={{ maxWidth: 'none' }}
            />
            
            {/* Close Button */}
            <button
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/80 hover:bg-black flex items-center justify-center border border-white/30"
              onClick={handleClose}
            >
              <img src="/assets/cross.svg" alt="Close" className="w-5 h-5" />
            </button>
          </div>
        </div>
      )}
