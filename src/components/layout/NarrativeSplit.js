...
function NarrativeSplit({
  ...
  if (layout === "scroll-gallery") {
    const gridCols =
      galleryImages.length >= 3 ? "md:grid-cols-3" : "md:grid-cols-2";
    return (
      <section className="max-w-5xl mx-auto py-6">
        ...
        {paragraph && (
          <p
            className={`mt-5 text-sm sm:text-base leading-[1.8] ${textColor} ${NARRATIVE_TEXT_WRAP}`}
          >
            {paragraph}
          </p>
        )}
      ...
      <section className="w-full my-24">
        ...
        >
          <CloudinaryImage
            ...
            className={`w-full h-auto block object-cover ${photoClass}`}
          />
          {onExpand && (
        ...
        {image.caption && (
          <div className={`pt-4 ${NARRATIVE_TEXT_WRAP}`}>
            <p className={`text-xs italic opacity-50 ${textColor}`}>
              {image.caption}
            </p>
          </div>
        )}
        {paragraph && (
          <div className={`pt-6 ${NARRATIVE_TEXT_WRAP}`}>
            <p className={`text-lg sm:text-xl leading-[1.8] ${textColor}`}>
              {paragraph}
            </p>
          </div>
        )}
      ...
      <figure className="feature-image w-full my-16">
        ...
        {image.caption && (
          <figcaption className="max-w-3xl mx-auto px-6 pt-4">
            <p className={`text-sm italic text-center ${textColor} opacity-70`}>
              {image.caption}
            </p>
          </figcaption>
        )}
  ...
  if (layout === "diptych") {
    ...
    return (
      <section className="max-w-5xl mx-auto py-6 px-3 sm:px-0">
        ...
        {paragraph && (
          <p
            className={`mt-5 text-sm leading-[1.8] ${textColor} opacity-80 ${NARRATIVE_TEXT_WRAP}`}
          >
            {paragraph}
          </p>
        )}
  ...
  if (layout === "editorial-split") {
    ...
    return (
      <section
        className={`editorial-split max-w-6xl mx-auto my-20 ${cardBg} backdrop-blur-sm`}
      >
        <div className="flex flex-col md:flex-row gap-8 md:gap-0">
          ...
          <div
            className={`${textWidth} w-full flex items-center px-6 md:px-8 py-10 md:py-14 ${textPadding}`}
          >
            <div>
              {eyebrow && (
                <p
                  className={`text-[10px] sm:text-xs font-bold uppercase tracking-[0.28em] mb-4 ${
                    variant === "dark"
              ...
              {paragraph && <p className={`${bodyClass} mb-0`}>{paragraph}</p>}
            </div>
          </div>
        </div>
      </section>
  ...
  if (layout === "asymmetric-focus") {
    const offsetDirection = imageLeft
      ? "md:ml-[10%] md:mr-auto"
      : "md:mr-[10%] md:ml-auto";
      ...
      <section className="asymmetric-focus max-w-5xl mx-auto my-24">
        {paragraph && (
          <div className="max-w-3xl mx-auto px-6 mb-10">
            ...
            {heading && <h3 className={`${headingClass} mb-6`}>{heading}</h3>}
            <p className={`${bodyClass} text-lg sm:text-xl leading-[1.8]`}>
              {paragraph}
            </p>
          </div>
        )}
        <div className={`w-full ${offsetDirection} max-w-[70%]`}>
          ...
          {image.caption && (
            <div className="max-w-[90%] mx-auto md:mx-0 pt-3">
              <p className={`text-xs italic ${textColor} opacity-60`}>
                {image.caption}
              </p>
            </div>
          )}
  ...
  if (layout === "immersive-break") {
    return (
      <section className="immersive-break w-full max-w-6xl mx-auto my-28">
        {paragraph && (
        ...
        <div
          className={`relative ${imgClass} shadow-2xl max-w-[26rem] sm:max-w-[34rem] md:max-w-[52rem] ${imageLeft ? "md:mr-auto md:ml-4" : "md:ml-auto md:mr-4"}`}
        >
          <CloudinaryImage
            legacyPath={image.src}
            alt={image.alt}
    ...
    return (
      <section className="split-offset max-w-6xl mx-auto my-20">
        ...
        >
          ...
          <div
            className={`md:w-[56%] w-full md:pl-8 lg:pl-12 ${imageLeft ? "" : "md:pr-8 lg:pr-12 md:pl-0"}`}
          >
            {eyebrow && (
              <p
                className={`text-[10px] sm:text-xs font-bold uppercase tracking-[0.28em] mb-4 ${
            ...
            {heading && <h3 className={`${headingClass} mb-6`}>{heading}</h3>}
            {paragraph && <p className={bodyClass}>{paragraph}</p>}
          </div>
        </div>
      </section>
    );
      ...
      <section className="float-inset max-w-[1000px] mx-auto my-16 px-6 md:px-4">
        <div className="berlin-float-inset" style={{ position: "relative" }}>
          ...
          >
            ...
            {image.caption && (
              <p className={`text-xs italic mt-2 ${textColor} opacity-60`}>
                {image.caption}
              </p>
            )}
          </div>
          ...
          {paragraph && (
            <p className={`${bodyClass} text-[1.15rem] leading-[1.8]`}>
              {paragraph}
            </p>
          )}
        </div>
    ...
    return (
      <section className={`max-w-5xl mx-auto pt-4 pb-6 ${NARRATIVE_TEXT_WRAP}`}>
        <div className="flow-root">
          ...
          />
          {paragraph && (
            <p className={`leading-[1.8] ${textColor} text-base md:text-lg`}>
              {paragraph}
            </p>
          )}
          {image.caption && (
            <p className={`mt-2 text-xs italic opacity-60 ${textColor}`}>
              {image.caption}
            </p>
          )}
        </div>
  ...
  const content = (
    <>
      ...
      <div className={`md:w-2/3 ${NARRATIVE_TEXT_WRAP} md:px-0 md:max-w-none`}>
        {eyebrow && (
          <p
            className={`text-[10px] sm:text-xs font-bold uppercase tracking-[0.28em] mb-2 ${
              variant === "dark"
                ? "text-editorialGold/90"
                : variant === "paper"
                  ? "text-[#6B5A49]"
        ...
        {heading && <h3 className={headingClass}>{heading}</h3>}
        {paragraph && (
          <p className={`${bodyClass} leading-[1.8]`}>{paragraph}</p>
        )}
      </div>
    </>
  );
  ...
  return (
    <section id={sectionId} className="max-w-5xl mx-auto py-10 scroll-mt-8">
      <div
        className={`flex flex-col md:flex-row gap-6 md:gap-10 items-center ${!imageLeft ? "md:flex-row-reverse" : ""}`}
      >
        {content}
      </div>
    </section>
