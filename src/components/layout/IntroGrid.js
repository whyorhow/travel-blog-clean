...
function IntroGrid({
  ...
  return (
    <section
      id={sectionId}
      className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-12 py-8 md:py-12 scroll-mt-8"
    >
      {/* Title */}
      <h1
        className={`text-4xl md:text-5xl font-semibold ${titleColor} mb-8 md:mb-10`}
      >
        {title}
      </h1>
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-10 items-start">
        ...
        <div className={`md:w-2/3 space-y-6 ${textPrimary}`}>
          {paragraphs.map((text, index) => (
            <p key={index} className={index === 0 ? leadClass : bodyClass}>
              {text}
            </p>
          ))}
        </div>
          ...
          <div className="md:w-1/3 w-full">
            <div className={`relative overflow-hidden ${tw.image}`}>
              ...
              <div
                ...
                style={{ backgroundColor: tokens.colors.overlay.imageTone }}
              />
            </div>
