...
import { hasAthensStaticHero } from "./utils/staticPageHero";
import {
...
/** Mobile /greece/athens — static HTML hero is LCP. */
export default function MobileAthensShellApp({ root }) {
  const [cookiesAccepted, setCookiesAccepted] = useState(null);
  const [PageComponent, setPageComponent] = useState(null);
  const staticHero = hasAthensStaticHero();
  const importPage = useCallback(() => import("./pages/AthensNew"), []);
  ...
  const upgradeToFullApp = useCallback(
    (location) => {
      import("./App").then(({ default: App }) => {
        ...
        if (location && location.pathname !== "/greece/athens") {
